...
export default function Auth() {
  ...
  const emailValid = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/.test(
    form.email.trim()
  );

  const normalizedPhone = form.telephone.replace(/[\s\-()]/g, "");

  const phoneValid = /^(032|033|034|037|038)\d{7}$/.test(
    normalizedPhone
  );

  const strength = useMemo(
    () => getStrength(form.motDePasse),
    [form.motDePasse]
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!form.telephone.trim() || !phoneValid) {
    ...
    <div className="min-h-[calc(100vh-61px)] grid md:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-6 py-12 bg-cream dark:bg-dark-bg transition-colors duration-300">
        <div className="w-full max-w-sm animate-fade-up">
          ...
          <h1 className="font-display text-3xl text-ink dark:text-white mb-1">
            {t.auth.welcome}
          </h1>

          <p className="text-sm text-ink-soft dark:text-gray-400 mb-6">
            {t.auth.subtitle}
          </p>
            ...
            {mode === "register" && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-semibold text-ink-soft dark:text-gray-400 mb-1.5">
                      {t.auth.nom}
                    </label>
                ...
                <div>
                  ...
                  <input
                    ...
                    type="tel"
                    inputMode="numeric"
                    placeholder="034 123 4567"
                    className={inputClass}
                  />

                  {form.telephone.length > 0 && !phoneValid && (
                ...
                <div className="mt-2">
                  ...
                  <p
                    className={`text-[11px] font-semibold ${
                      STRENGTH_STYLES[strength].label
                    }`}
                  >
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className="text-[10px] text-ink-soft dark:text-gray-500 uppercase tracking-widest">
              ou
            </span>
            <span className="flex-1 h-px bg-line dark:bg-dark-line" />
          </div>
